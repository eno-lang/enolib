...
import { cursor, DOCUMENT_BEGIN, selection, selectComments, selectElement, selectKey } from './selections.js';
import {
    ATTRIBUTE,
    BEGIN,
    END,
...
export const errors = {
    ...
    missingValue: (context, element) => {
        ...
        if (element.type === FIELD || element.type === EMBED_BEGIN) {
        ...
        if (element.type === FIELD && element.hasOwnProperty('continuations')) {
