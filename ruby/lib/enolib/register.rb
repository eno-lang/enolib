...
module Enolib
  def self.register(definitions)
    definitions.each do |name, loader|
      ...
      Field.send(:define_method, "optional_#{name}_values") { optional_values(loader) }
      Field.send(:define_method, "required_#{name}_values") { required_values(loader) }
      ...
      MissingField.send(:alias_method, "optional_#{name}_values", :optional_string_values)
      MissingField.send(:alias_method, "required_#{name}_values", :required_string_values)
    end
  end
end
